...
const [loading, setLoading] = useState(false); // Introduce loading state
  ...
  const handleSubmit = async (e) => {
    ...
    if (store.loggedIn) {
      ...
      const response = await fetch(`http://65.1.16.209:8080/user/loggedIn/customerService`, {
        method: "POST",
        headers: {
      ...
      const response = await fetch(`http://65.1.16.209:8080/guest/customerService/message`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "subject": formState.subject,

...
const [loading, setLoading] = useState(false); // Introduce loading state
  ...
  const handleSubmit = async (e) => {
    ...
    if (store.loggedIn) {
      ...
      const response = await fetch(`https://13.234.225.97:8080/user/loggedIn/customerService`, {
        method: "POST",
        headers: {
      ...
      const response = await fetch(`https://13.234.225.97:8080/guest/customerService/message`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "subject": formState.subject,

...
const ROOT = "https://13.234.225.97:8080";
...
const Admin = () => {
  ...
  const handleCancelOrder = async () => {
    ...
    try {
      const response = await fetch(`https://13.234.225.97:8080/order/${orderNrCancel}`, {
        method: 'DELETE',
        headers: {
  ...
  const handleLogout = async () => {
    ...
    try {
        ...
        const response = await fetch("https://13.234.225.97:8080/admin/signOut", {
            method: 'DELETE',
            headers: {
                'Content-Type': 'application/json',
                'x-auth-token': authToken,
...
const handleMarkSent = async () => {
  setLoading(true);
  try {
    const response = await fetch(`https://13.234.225.97:8080/order/sent/${orderNrSent}/${trackingId}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
        'email': adminEmail,
  ...
  const handleMarkDelivered = async () => {
    setLoading(true);
    try {
      const response = await fetch(`https://13.234.225.97:8080/order/delivered/${orderNrDelivered}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'email': adminEmail, // Use the provided admin email for authentication
